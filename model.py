...
class NeuralNetwork:
	def __init__(self, inp_shape, class_amount, max_output=25, lr=1e-1, momentum=0.9, confidence_weight=0.7, load=False):
		...
		head_outputs = [[], []]
		for prior, output in zip(priors, outputs):
			location_pred = Conv2D(filters=prior * 4, kernel_size=(3, 3), strides=(1, 1), padding="same")(output)
			location_pred = Reshape((-1, 4))(location_pred)
			head_outputs[0].append(location_pred)

			class_pred = Conv2D(filters=prior * class_amount, kernel_size=(3, 3), strides=(1, 1), padding="same")(output)
	...
	def get_preds(self, data):
		...
		selected_classes, selected_boxes, selected_scores = self.postprocessing(bbox_preds[0], class_preds[0])
